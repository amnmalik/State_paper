...
# Step 0: Load the GCAM database and set up project
...
# Step 1: Load real generation data for 2020
...
# Step 2: Get GCAM electricity generation data
database_data <- getQuery(prj, query = 'elec gen by subsector', scenario = "NZ_Final") %>%
  filter(year >= 2020) %>%
...
# ⚠️ Ensure only one row per (region, subsector, year)
database_data_filtered <- database_data_filtered %>%
  group_by(region, subsector, year) %>%
  summarise(value = sum(value), .groups = "drop")

# Step 3: Compute 2020 difference (real vs model)
...
# ⚠️ Keep only one row per (region, subsector)
database_2020_dedup <- database_2020 %>%
  distinct(region, subsector, .keep_all = TRUE)

# Step 4: Create full year-subsector-state matrix
...
# Step 5: Merge and apply adjustment
...
write.xlsx(adjusted_data, "C:/Users/Anurag/Downloads/adjusted_generation_forecast_NZ.xlsx")

# Step 7: Filter for selected states and techs
