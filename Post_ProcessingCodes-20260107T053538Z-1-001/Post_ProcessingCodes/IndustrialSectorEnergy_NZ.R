...
database_data <- getQuery(prj, query = 'final energy consumption by sector and fuel',scenario = "NZ_Final") %>%
...
adjusted_data <- database_aggregated %>%
  ...
  filter(year >= 2020 & year <= 2070 & year %% 5 == 0) %>%  # Filter for 5-year intervals
  left_join(difference_data, by = c("region", "input")) %>%
  mutate(
    adjusted_value = ifelse(year <= 2050, total_value + coalesce(difference, 0), total_value), # Add difference to total_value
    adjusted_value = if_else(adjusted_value < 0, total_value, adjusted_value)  # Replace negative values with total_value
  ) %>%
  select(region, input, year, adjusted_value) %>%  # Select relevant columns
  arrange(region, input, year)  # Arrange data
...
#  left_join(difference_data, by = c("region", "input")) %>%
#  mutate(
#    adjusted_value = ifelse(year <= 2050, total_value + coalesce(difference, 0), total_value) # Add the difference to database values
#    has_negative = any(adjusted_value < 0),
#     adjusted_value = ifelse(has_negative, total_value, adjusted_value)
#    ) %>%
...
output_path <- "C:/Users/Anurag/Downloads/adjusted_industryenergy_NZ.xlsx"
